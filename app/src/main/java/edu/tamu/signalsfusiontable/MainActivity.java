package edu.tamu.signalsfusiontable;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String APPLICATION_NAME = "signals-1270";   // Set the Application Name
    ...
    //	----------------------------------------------------------------------------------------------------------------
    public boolean AddNewEntry(Sigdata signal)
    {
        boolean status = false;                                         // Return status (success / failure)

        try                                                             // Try to get JSON, and save data to database
        {                                                               //
            if (signal.ID != -1)                                  // If not default then save data
            {
                // Print debug information to port
                System.out.println("Insert New RF Data into Table - Signal ID: " + signal.ID + ", Strength: " + signal.Strength + ", Location: " + signal.Latitude + ", " + signal.Longitude);

                SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String sql_string;                                      // Build up SQL string
                // Add to fusion table
                String tableId = GetTableId("Signals");       // Get the "Customer Data" Table
                sql_string = "INSERT INTO " + tableId + " (";       // Insert SQL statement, Table: Table ID
                sql_string += "ID,";                            // Field: intXbeeID
